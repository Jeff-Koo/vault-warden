...
if [[ -f "$PROJECT_DIR/.env" ]]; then
    ...
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] ERROR: Missing .env file at $PROJECT_DIR/.env" >&2; exit 1
fi
...
if [[ ! -s "$DUMP_FILE" ]]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] ERROR: Database dump is empty or failed!" >&2
    exit 1
fi

### Execute Restic incremental backup
log_info "[2/5] Uploading to Cloudflare R2..."
...
if [[ $RESTIC_RC -ne 0 && $RESTIC_RC -ne 3 ]]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] ERROR: Restic backup failed with exit code $RESTIC_RC" >&2
    exit $RESTIC_RC
fi

### Automatic cleanup of expired snapshots
log_info "[3/5] Running retention (7D, 4W, 6M)..."
restic "${RESTIC_OPTS[@]}" forget --keep-daily 7 --keep-weekly 4 --keep-monthly 6 --prune --quiet

### Periodic health check
log_info "[4/5] Verifying repository integrity..."
restic "${RESTIC_OPTS[@]}" check --read-data-subset=10% --quiet

log_info "[5/5] Done! Backup successfully synced to Cloudflare R2."
