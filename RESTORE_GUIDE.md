...
3. **驗證標籤（Labels）：** 確保本專案的 `compose.yaml` 內，關於 Traefik 的 `labels` 設定與該倉庫定義的進入點（Entrypoints）與網路（Docker Network）完全一致。
4. **檢查憑證：** 確保外部的 `443` 加密流量能透過 Traefik 正確導向本機的 Vaultwarden 容器，並成功由 Let's Encrypt 簽發新伺服器環境的 SSL 憑證。
